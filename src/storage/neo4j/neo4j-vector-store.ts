...
export class Neo4jVectorStore implements VectorStore {
  ...
  async search(
    ...
    try {
      ...
      if (!hasValidNorm) {
        logger.warn(
          "Neo4jVectorStore: Vector has invalid l2-norm, using fallback search"
        )
      ...
      try {
        ...
        logger.debug(
          `Neo4jVectorStore: Vector search found ${foundResults} results`
        )
        ...
        logger.debug(
          "Neo4jVectorStore: No results from vector search, using fallback"
        )
