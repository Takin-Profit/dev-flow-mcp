 ...
 */
import { type Relation, RelationValidator } from "#types/relation.ts"
...
export class TemporalRelationValidator {
  ...
  static isTemporalRelation(obj: unknown): boolean {
    ...
    if (
      temporalObj.validFrom !== undefined &&
      typeof temporalObj.validFrom !== "number"
    ) {
      return false
    }

    if (
      temporalObj.validTo !== undefined &&
      typeof temporalObj.validTo !== "number"
    ) {
      return false
    }

    if (
      temporalObj.changedBy !== undefined &&
      typeof temporalObj.changedBy !== "string"
    ) {
      return false
    }
